...
def MovementVRF():
    def generate():
        ...
        while 1==1:
            log_msg = "Idle"
            s_sql = "select * from AIS_Monitoring where AMT_Setting = 'MovementVRF'"
            # print(s_sql)
...
def payment():
    def generate():
        ...
        while 1==1:
            log_msg = "Idle"
            s_sql = "select * from AIS_Monitoring where AMT_Setting = 'Invoice/Payment'"
            # print(s_sql)
            msg, data_rows, errFlag = db.exec_query(engine, s_sql)
            if data_rows is not None:
                rows = data_rows.to_json(orient='records')
...
def movementrequest():
    def generate():
        ...
        while 1==1:
            log_msg = "Idle"
            s_sql = "select * from AIS_Monitoring where AMT_Setting = 'MovementRequest'"
            #print(s_sql)
            msg, data_rows, errFlag = db.exec_query(engine, s_sql)
            if data_rows is not None:
                rows = data_rows.to_json(orient='records')
